...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/">
    <table width='98%' align='center' id="weely-volume" class='table-account'>
      <xsl:for-each select="resptable/tabletitle">
        <tr class="trow-title">
          <th width="33%" align='center'><xsl:value-of select="coltitle1"/></th>
          <th width="33%"><xsl:value-of select="coltitle2"/></th>
          <th width="34%"><xsl:value-of select="coltitle3"/></th>
        </tr>
      </xsl:for-each>
